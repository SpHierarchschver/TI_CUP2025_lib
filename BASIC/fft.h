...
void rfft (float32_t input[], int N, complex_t cmplxOut[], float32_t outputMagnitude[], WindowType winType);  // Real FFT.
void rifft (const float32_t outputMagnitude[], int N, float32_t time_data[]);           // Real inverse FFT.

/* For debug. */
void cifft_test (complex_t x[], int N, float32_t time_data[]);
void cifft_local (int N, float32_t time_data[]);
